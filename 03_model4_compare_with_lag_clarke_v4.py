...
class CNN_BiLSTM_Att_Model(nn.Module):
    def __init__(self, input_size):
        ...
        # 1. 卷积层: 1x1卷积核, 64个滤波器, 提取跨通道特征 (参照原论文表5-1)
        self.conv1 = nn.Conv1d(in_channels=input_size, out_channels=64, kernel_size=1)
        self.relu = nn.ReLU()

        # 2. Bi-LSTM 层: 两层双向 LSTM (参照原论文表5-1: 第一层40, 第二层120)
        # PyTorch中 hidden_size 是单向的，所以 20*2=40, 60*2=120
    ...
    def forward(self, x):
        # 记录当前窗口最后一个真实血糖值 (用于残差计算)
        last_known_cgm = x[:, -1, 0].unsqueeze(1)

        # Conv1d 要求输入格式为 (batch, channels, seq_len)
        x_conv = x.permute(0, 2, 1)
        x_conv = self.relu(self.conv1(x_conv))
        x_conv = x_conv.permute(0, 2, 1)  # 转换回 (batch, seq_len, channels)

        # Bi-LSTM 特征提取
        out, _ = self.bilstm1(x_conv)
        out, _ = self.bilstm2(out)  # out shape: (batch, seq_len, 120)

        # Attention 权重计算与上下文向量加权
        weights = self.attention_weights(out)  # shape: (batch, seq_len, 1)
        weights = torch.softmax(weights, dim=1)
        context = torch.sum(weights * out, dim=1)  # shape: (batch, 120)
...
if __name__ == "__main__":
    ...
    # 👇【修复3】在这里把新加的模型也配置进字典里，否则会 KeyError 报错崩溃！
    ceg_data = {
        "SVR": {"true": [], "pred": []},
        "1D-CNN": {"true": [], "pred": []},
        "CNN-BiLSTM-Att (Liang)": {"true": [], "pred": []},
            ...
            for model_name in MODELS_TO_COMPARE:
                ...
                for f_path in test_files:
                    try:
                        ...
                        elif model_name == "CNN-BiLSTM-Att (Liang)":
                            model = CNN_BiLSTM_Att_Model(input_dim)
                            # 👇【修复1&2】修正为对齐这个脚本的数据流，删除画蛇添足的逆变换
    ...
    print("\n" + "=" * 100)
    print("。。。 正在基于真实的、患者级隔离环境 生成四大模型的临床安全性评价图 (Clarke Error Grid)...")
    for m_name in MODELS_TO_COMPARE:
        t_vals = ceg_data[m_name]["true"]
        p_vals = ceg_data[m_name]["pred"]
        if len(t_vals) > 0:
            safe_name = m_name.replace(" (Ours)", "").replace("-", "_").replace(" ", "_").replace("_(Liang)", "")
